...
<html lang="en">
  <head>
    ...
    <title>Filmes Listados - Neoxtv </title>
  </head>
  <body>
...
if (!empty($_POST["dataForRemoving"])){
  ...
  $id_filme = $_POST["dataForRemoving"];
  $selecionando = "SELECT path FROM filmes WHERE id_filme = $id_filme";
  ...
  $query_for_removing = "DELETE FROM filmes WHERE id_filme='$id_filme' "; 
  mysqli_query($conn,$query_for_removing);
  unlink("$capa");

}

$query = "SELECT id_filme,nome_f,path,ano_public_f,genero_f,cla_etaria_f,descricao_f,link_f FROM filmes";
...
while($linha = mysqli_fetch_array($resultado)){
    echo "<tr> <td>".$linha['id_filme']."</td>
    <td><img height='90px' class='rounded' src=".$linha['path']."></td>
    <td>".$linha['nome_f']."</td>
    <td>".$linha['ano_public_f']."</td>
    <td>".$linha['genero_f']."</td>
    <td>".$linha['cla_etaria_f']."</td>
    <td>".$linha['link_f']."</td>
    <td class='tam'>".$linha['descricao_f']."</td>";
    ?>

    <td>
    <form method = "post" action="update.php">
    <input type = "hidden" id="inputHidden" name="dataForUpdating" value=<?php echo $linha['id_filme']; ?> >  
      <button type = "submit" class="btn btn-info btn-xs"  >Editar</button> 
    </td>
    </form>  

    <td>
    <form method = "post" action="listadoFilme.php">
    <input type = "hidden" id="inputHidden" name="dataForRemoving" value=<?php echo $linha['id_filme']; ?> >  
